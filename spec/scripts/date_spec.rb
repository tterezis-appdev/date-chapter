describe "formatted.rb" do
  it "outputs 'The year is: 2020, the calendar day is: 1, and the month is: 7.' when today is July 1, 2020", points: 1 do
    ...
    output = with_captured_stdout { require_relative('../../formatted')} 
...
describe "formatted.rb" do
  it "outputs the formatted dates for any past/future date", points: 1 do
    # Un-require formatted.rb
    formatted_file = $".select{|r| r.include? 'formatted.rb'}
    $".delete(formatted_file.first)
...
describe "math.rb" do
  it "is not 'hard-coded' to the current date'", points: 1 do
    # Un-require math.rb
    math_file = $".select{|r| r.include? 'math.rb'}
    $".delete(math_file.first)
...
describe "monday.rb" do
  it "should output 'Is today Monday? true', when today is Monday", points: 1 do
    ...
    output = with_captured_stdout { require_relative('../../monday')} 
    output = "empty" if output.empty? 
...
describe "monday.rb" do
  it "should output 'Is today Monday? false', when today is not Monday", points: 1 do
    # Un-require monday.rb
    monday_file = $".select{|r| r.include? 'monday.rb'}
    $".delete(monday_file.first)
